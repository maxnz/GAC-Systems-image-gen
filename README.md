# Systems 1 & 2 Image Generation using `pi-gen`
This is a set of files that configure [`pi-gen`](https://github.com/RPi-Distro/pi-gen) to create a custom image for Systems 1 & 2 at Gustavus Adolphus College.

## General Overview
This modification to `pi-gen` sets up everything for the Systems 1 & 2 image.

## Requirements
- Raspberry Pi 3B/3B+/4B running Raspbian Trixie
...
git clone https://github.com/maxnz/systems-1-2-gen.git
```

- Install missing dependencies
```bash
sudo apt update
sudo apt install quilt qemu-user-binfmt debootstrap zerofree libarchive-tools xxd pigz arch-test
```

- Copy modifications into `pi-gen`
```bash
cp -r systems-1-2-gen/pi-gen/* pi-gen/
```
